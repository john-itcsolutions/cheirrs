...
The Internet of Things (IoT) system revolves around the IoT server on worker-2 being in communication with smart-web via gRPC protocol buffers installed in both machines (pyhon-grpc-adenine client repo from elastos). The smart-web server passes on IoT events to both blockchains and database. The actual "Things" generating messages and signals are connected at the sites by raspberry-pi "edge" clients or similar.
...
`juju config pg-a admin_addresses=127.0.0.1,0.0.0.0,<ip-addr-worker-0>`
...
<img src="./Screenshot from 2021-07-04 12-56-11.png">
