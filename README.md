...
gRPC protocols replace the older style REST APIs, for communicating requests, and data inputs, from the client and responses from the blockchain &/or database back to the client; all via the smart-web server. Here the gRPC protocols are implemented in Python.
